...
void PublishedCmdsISR(LIN_Device_Frame_t *const frame)
{
    ...
    if (frame->frame_id == CmdsTable[FID_STATUS_FRAME_INDEX].frame_id){
        ...
        frame->data[0] = ((l_bool_rd_LI0_C_3way2_C_LinError()) | (l_u8_rd_LI0_C_3way2_C_InitialSta()<<2) | (l_bool_rd_LI0_C_3way2_C_Running()<<3));
        frame->data[1] = ((l_u8_rd_LI0_C_3way2_C_Fault())|(l_u8_rd_LI0_C_3way2_C_Voltage()<<8)|(l_u8_rd_LI0_C_3way2_C_OverTemp()<<12));
        frame->data[2] = (l_u8_rd_LI0_C_3way2_C_PositionFB()); //ls_read_error_code();
        frame->data[3] = (l_u8_rd_LI0_C_3way2_C_SW_Ver()); // MES_DataIntegrityError() | ((uint8_t)SAFM_GetBatteryState() << 2) | ((uint8_t)SAFM_GetChipTemperatureState() << 4);
        frame->data[4] = (l_u8_rd_LI0_C_3way2_C_HW_Ver()); // MES_DataIntegrityError() | ((uint8_t)SAFM_GetBatteryState() << 2) | ((uint8_t)SAFM_GetChipTemperatureState() << 4);
...
void LINS_TaskHandler(void)
{
    ...
    case TASK_STATE_ACTIVE:
      if (receivedFrame.frame_id == CmdsTable[FID_COLOR_CTRL_INDEX].frame_id){
        l_u8_wr_LIN_C_3way_PosReq(receivedFrame.data[0]<<8|receivedFrame.data[1]);
        l_bool_wr_LIN_C_3way2_MoveEnable(receivedFrame.data[2]);
        l_u8_wr_LIN_C_3way_Initial(receivedFrame.data[3]<<8|receivedFrame.data[4]);
        l_bool_wr_LIN_C_3way2_LinError(receivedFrame.data[5]);
    ...
    case TASK_STATE_INIT:
      ...
      l_u8_wr_LIN_Valve_ND_CR_Valve1_HWversion(15);
      l_u8_wr_LIN_Valve_ND_CR_Valve1_SWversion(14);
      l_u8_wr_LIN_Valve_ND_CR_Valve1_Res1_HeaderID(0x12);
      l_u8_wr_LIN_Valve_ND_CR_Valve1_Res2_HeaderID(0x22);      
