...
static LIN_Device_Frame_t UnconditionalCmdsTable[DEFAULT_LINS_FID_SIZE] = {
  ...
  [FID_EXVD_DATC_INDEX] = {
      .frame_id = ExvD_Datc,   /* status management frame */
...
void UnconditionalSubscribedCmdsHandle(LIN_Device_Frame_t const *frame)
{
    if (frame->frame_id == UnconditionalCmdsTable[FID_DATC_EXVD_INDEX].frame_id)
    {
        ...
        l_u8_wr_LIN_Valve_ND_CR_Valve1_Res1_HeaderID(frame->data[0]);
        l_u8_wr_LIN_Valve_ND_CR_Bms_cmdAngle_Valve(frame->data[1]);
        l_u8_wr_LIN_Valve_ND_CR_Valve_BmsCmdAngle(frame->data[2]);
        l_u8_wr_LIN_Valve_ND_CR_Bms_Angle_Valve_FB(frame->data[3]);
        l_u8_wr_LIN_Valve_ND_CR_Valve_Status_FB(frame->data[4]);
