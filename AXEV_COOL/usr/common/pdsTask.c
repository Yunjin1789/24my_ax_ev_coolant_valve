...
{
    CoolValveInfo.sChecksum.u8SleepChecksum = 0xAA;
    FLASH_StoreAll();
}
...
void FLASH_Read(void)
{
    ...
    if(CoolValveInfo.sChecksum.u8SleepChecksum == 0xAA){
        memset(&CoolValveInfo.uFaultFlag, 0, sizeof(CoolValveInfo.uFaultFlag));
        memset(&CoolValveInfo.sAdc, 0, sizeof(CoolValveInfo.sAdc));
        memset(&CoolValveInfo.sMotorDriver, 0, sizeof(CoolValveInfo.sMotorDriver));
        memset(&CoolValveInfo.sChecksum, 0, sizeof(CoolValveInfo.sChecksum));
    }
    else{
...
{
    CoolValveInfo.sMotor.u8ValveWakeupMode = (uint8_t)DISABLED;
    FLASH_Save();
}
...
/*  *************************fix color mixing  configuration default value********************* */
        ...
        for (uint8_t i = 0U; i < CFL_LED_NUM; i++){
            ...
            systemParam.info.ledParams.led[i].ledTypicalPNVolt[LED_RED]         = DEFAULT_LED_PN_VOLT_R;
            systemParam.info.ledParams.led[i].ledTypicalPNVolt[LED_GREEN]       = DEFAULT_LED_PN_VOLT_G;
            systemParam.info.ledParams.led[i].ledTypicalPNVolt[LED_BLUE]        = DEFAULT_LED_PN_VOLT_B;
        }
    }
